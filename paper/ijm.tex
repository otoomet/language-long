...
\usepackage[margin=3cm,top=2.5cm, bottom=2.0cm]{geometry}
...
\newcommand{\modelTwo}{age, age\textsuperscript{2}, education, being married, number of children, inter-ethnic household}
...
\newcommand{\restrictions}{The sample period is from year 2000 to year 2012. Sample is limited to persons
	25-55 year old.}
\newcommand{\agerestrictions}{Sample is limited to persons
	25-55 year old.}
%Modified section numbering	
...
\begin{equation}
	...
	\begin{split}
		y_{it} ^{g} = &\: \alpha^{g}{}' X_{it}^{g} + \vec{\beta}^{g}{}' \cdot \vec{L}_{it}^{g} +
		\eta_{i}^{g} +
...
\fxnote*{Did we estimate it really that way? At least not in my code. I found the code for estimations with these interactions, but I don't remember that I used these interactions for the tables.}{corresponding interactions}
\begin{align}
...
Finally, the third model additionally includes industry, occupation, and other
workplace descriptors.
\Cref{tab:models} summarises this description of the models.
%SKB: This table should have been in the article at some point, but I must have removed it. I use in my Quantile Regression article
\begin{table}[hbtp]
	\centering
	\caption{Groups of explanatory variables included in different models}
	\label{tab:models}
	\begin{tabular}{r|ccc}
		\toprule
...
\cref{tab:unemployment_estimation_by_sex_and_ethnic_same_sample}.
\footnote{This table is restricted to have the same number of observations across all models requested by reviewer. The unrestricted table is in the appendix \cref{tab:unemployment_estimation_by_sex_and_ethnic}.}
...
\cref{tab:wage_estimation_by_sex_and_ethnic_same_sample} presents the main
results.\footnote{This table is restricted to have the same number of observations across all models requested by reviewer. The unrestricted table is in the appendix \cref{tab:wage_estimation_by_sex_and_ethnic}.}
...
individual performance. Adding further observable workplace characteristics nearly halves the effects 
(when accounting not only for the industry but also the occupation) %SKB: Might remove this sentence
.
...
We estimate the effect of language skills for unemployment for the age groups and ethnical groups as well for the second model. However dividing the results for four sub-periods does not detect the trends across the age groups for those periods due to only some of the results are statistically significant. The exception is the group of young (25-34) Estonian men for whom English language is clearly decreases the probability of being unemployment during the time. Notwithstanding importance of English is small for other groups.
Estimates for the longer time period shows the importance of Estonian language for all age groups for ethnical Russian and have even higher impact for the young women.
