...
\begin{equation}
	...
	\end{split}
\end{equation}
where $y_{it}$ is the outcome (either unemployment indicator
or log-wage)
of individual $i$ in wave $t$; $RUS$, $EST$, and $ENG$ are the
language skill descriptors; $\vec{X}$ are the other individual
characteristics, $\upsilon_{t}$ are the year dummies and $\epsilon_{it}$
is the idiosyncratic error term.  As we use panel data, we cluster the
