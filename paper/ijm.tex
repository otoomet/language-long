...
We use the same econometric specification as described in section \ref{sec:method}, the results presented here are derived using the model 2.
...
young groups, the age groups where the Russian skills are also rapidly declining.
Russian is still closely related to salary for
older workers. The importance of English language skills fluctuates
and declines slowly through the period, just the oldest male group
(45-55) and the youngest female group (25-35) show a growing trend.
...
statistically significant results at all.  We'll return to this
outcome below.
The positive impact of Estonian language skills for Russian women
increases
over the sample period for the young group (25--35)
and the prime age group (36--45).
Good
Estonian language knowledge is associated with approximately 10\% higher salary for Russian women for the boom in the 2000s. For the
...
Russians, at least women, with a good Estonian command can get well-paid jobs.
In comparison, the estimates for English language fluency shows
downward trend for all age groups for men and for women.  A likely
...
We repeat the exercise for unemployment models (using specification 2
in
Table~\ref{tab:unemployment_estimation_by_sex_and_ethnic_same_sample}).
However, as most of the results are not statistically significant, we
do not present the tables here.
...
The most interesting result in our analysis so far--the fact that
Russian men earn virtuannly no Estonian language premium--warrants a
deeper look at the data.  We split the Russian male wage-earners' data according to three
dimensions: Estonian language skills, residence in Tallinn metro area,
and education.  However, to correct very strong trends through these
years both in wage and language skills, we present the average
...
\fxwarning{ot: Sven, can you redo this table?  Use exactly the same
  ...
  the same row}
The table show that the picture is in fact more complex.  We can see
...
the metropolitan area, the the lowest and highest education groups
...
most controversial finding, no positive effect of Estonian language
skills for Russian men, is primarily driven by workers with
high-school degree.

All previous results can be explained by two hypotheses: first, Russian--speaking men are working
