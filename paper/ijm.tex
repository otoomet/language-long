...
\fxwarning{SKB: Let's collect first enough material and then decide, but I can start adding here some notes from \cite{Kahanec2009,Kahanec2010}.}
%Start here CEE stuff for now
...
\cite{Alan2015} \enquote{During the Soviet era, proficiency in the Russian language was often a ticket to
	attractive employment opportunities in the member republics. Does it still contribute
	to securing employment in the former Soviet republics after two decades of transition?
	Using data from Armenia, Azerbaijan and Georgia in the years 2008–2010,
	this paper demonstrates that Russian language skills remain economically valuable.
	The baseline estimates suggest that Russian language skills increase probability of
	employment by about 6 (males) and 9 (females) percentage points. Our results bear
	important implications for the ongoing debates on language policies in the post-
	Soviet countries.} \fxnote{SKB: Quite detailed article about Russian language proficiency in three former Soviet Union states. Needs further comments from us and find similar articles.}
...
\fxnote{\cite{Kahanec2009} essentially confirms your results in \cite{Leping2008,Toomet2011}}
...
the mid-1990s.  \Cref{fig:gdp_growth} indicates that the Estonian economy grew around 7\% per year before the financial crisis in 2008 and after that grew at a lower rate around 3\% per year.
\fxwarning{ot: a few numbers about economic growth would be good}
\fxwarning{SKB: I added rather a figure. Pure numbers or table might not give the best impression of the growth.}
\begin{figure}[h]
	\centering
...
\fxwarning{SKB: What do you mean with time trends? Which value should a ``year'' variable take? Give me an example, since I don't understand what you want. }
...
\fxerror{SKB: Eta should be the individual specific random effect. We cluster the standard errors at the individual level, hence estimate a random-effect model, although not a panel random-effect model.}
\fxerror{ot: why does $\upsilon$ have index $g$?}
\fxerror{SKB: Should be an error, probably index $g$ was just added to everything.}
...
\fxnote{Actually, I tried to estimate Model 3 as well for unemployment probability. The language coefficients become insignificant after controlling for industry and occupation. Hence, Estonian language fluency lowers the unemployment probability only if workplace characteristics are ignored. Since nobody questioned this result during review, we should maybe not mention it now. }
...
\fxnote{SKB: I do not remember such request. The only request, that I remember, is to make the scaling of the figures equal or cut off parts of the large confidence intervals, so that the scaling of the y-axis becomes smaller/more detailed.}
...
\fxwarning{ot: Sven, can you redo this table?  Use exactly the same
  ...
  the same row}
\fxwarning{SKB: Will try. Although I still do not understand, what the residual is telling me and why the wages are so high. Are the wages in EEK? What does ET stand for? The table needs more explanation or at least a proper legend.}
...
\end{table}
\fxwarning{SKB: Copied and modified table above. }
\begin{table}[ht]
	\centering
	\caption{Average wage and time-trend corrected language parameter across
		location and education groups for Russian men}
	\begin{tabular}{lrlrrr}
		\toprule
		Tallinn & education & ET & wage & parameter & \# obs \\ 
		\midrule
		Tallinn & $<$ HS & 0 & 334 & -0.16\onepc & 255 \\ 
		Tallinn & $<$ HS & 1 & 424 & -0.08 & 219 \\ 
%		Tallinn & HS     & 0 &476 & 0.05 & 1292 \\ 
		Tallinn & HS      & 1 & 512 & -0.05 \fivepc & 1296 \\ 
		Tallinn & college & 0 & 520 & -0.02 & 212 \\ 
		Tallinn & college & 1 & 737 & 0.05 & 518 \\ 
		Ida-Viru & $<$ HS & 0 & 396 &  -0.20\onepc & 379 \\ 
		Ida-Viru & $<$ HS & 1 & 269 & -0.23\onepc & 20 \\ 
		Ida-Viru & HS     & 0 & 337 & -0.23\onepc & 1889 \\ 
		Ida-Viru & HS      & 1 & 368 & -0.21\onepc & 252 \\ 
		Ida-Viru & college & 0 & 451 & -0.07 & 246 \\ 
		Ida-Viru & college & 1 & 493 & -0.06 & 154 \\ 
		other   & $<$ HS & 0 & 279 & -0.23\onepc & 83 \\ 
		other   & $<$ HS & 1 & 334 & -0.19\onepc & 212 \\ 
		other   & HS     & 0 & 378& -0.13\onepc & 353 \\ 
		other   & HS      & 1 & 389 & -0.09\onepc & 713\\ 
		other   & college & 0 & 788 & 0.03 & 32\\ 
		other   & college & 1 & 414 & -0.11 & 110 \\ 
		\bottomrule
	\end{tabular}
	\label{tab:loc-lang-edu_m3}
\end{table}

\fxwarning{SKB: Added graph showing the same parameter estimates. Maybe a better formatted graph like this shows the story better than a table}
\begin{figure}[hbtp]
	\centering
	\caption{Coefficient plot of Model 3 for Russian Men separated by Region and Education}
	\label{fig:russianmenlanguagecoefsm3}
	\includegraphics[width=1.0\linewidth]{russian_men_language_coefs_m3}
\end{figure}
...
that.}
\fxwarning{SKB: You removed the cv.ee already? I only see stuff using data from Integration Monitor Survey 2008. I did not do these analyses nor do I have the data to repeat the analyses for the latest edition of the Integration Monitor Survey.
The cv.ee might have been interesting or useful, if it was done properly documented. But that might warrant another section or even another paper. See \cite{fabo+2017E} for more on that.
}
